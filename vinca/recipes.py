"""Generation of package outputs for rattler-build recipes."""
...
def get_depmods(vinca_conf: dict[str, Any], package_name: str, distro: Any):
...
def _apply_depmods(requirements: dict[str, list], removed: dict, added: dict) -> None:
...
def _adjust_requirements(requirements: dict[str, list], package_prefix: str) -> None:
...
def generate_output(
    ...
    if not xml:
        print(
            f"Skip {shortname} as it is present in our snapshot, but not in the "
            "latest rosdistro cache."
        )
        return None
